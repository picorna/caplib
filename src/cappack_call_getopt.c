...
/*	int *f_runtype;
		// (0) Default: analyse the rotation axes and angles
		// (1) add water region in the central cell of 2PLV using the wat216.dat file
		// (2) generate entire capsids from files (no param, f_runtype == 2)
		// (3) print center
		// (4) -r overlay a PDB onto another PDB (rotate a PDB file)
*/
/*
	int *f_verbose;
		// output loudly (-v, f_verbose  == 1)
	int *f_option_m;
		// The BIOMT lines are read from a separate file.
	int *f_option_t;
		// No BIOMT lines are used. Instead, the standard matrices of 2PLV or 4RHV are used.
		// f_option_t == 1 --> 2PLV matrices are used.
		// f_option_t == 2 --> 4RHV matrices are used.
		// f_option_t == 0 --> the -t option is not used.
        int f_option_c;
		// The cell list is read and stored in cells[60]
*/   
	*f_verbose  = 0;
	*f_option_m = 0;
	*f_option_t = 0;
