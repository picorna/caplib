...
caplib.py is a file composed of Capgen, Caprad, Caprot, Capwat, Capswch, Captwst, get_obj_center and setaxis.
...
'''
def capgen(shift = 0,file_name1="NotInputFileName"):
...
#    cdll.LoadLibrary("/Users/yoneda/Desktop/caplib/Python/capgen5.so")
#    libc = CDLL("/Users/yoneda/Desktop/caplib/Python/capgen5.so")
    if file_name1 == "NotInputFileName":
        file_name = c_char * 20; file_name1 = file_name()
        file_name1 = objname[0] + ".ent"
...
#    libc.read_biomt_file(c_char_p(file_name1), byref(rot_array), byref(xyzcent_t_array))
    ...
    for i in range(len(lines)):
        line = lines[i]
        if line[0:19] == "REMARK 350   BIOMT1":
            rot_mat[0][0] = float(line[24:33])
            rot_mat[0][1] = float(line[34:43])
            rot_mat[0][2] = float(line[44:53])
            shf[0]      = float(line[54:69])
        if line[0:19] == "REMARK 350   BIOMT2":
            rot_mat[1][0] = float(line[24:33])
            rot_mat[1][1] = float(line[34:43])
            rot_mat[1][2] = float(line[44:53])
            shf[1]      = float(line[54:69])
        if line[0:19] == "REMARK 350   BIOMT3":
            rot_mat[2][0] = float(line[24:33])
            rot_mat[2][1] = float(line[34:43])
            rot_mat[2][2] = float(line[44:53])
            shf[2]      = float(line[54:69])
            rot_mats.append(copy.deepcopy(rot_mat))
            shfs.append(copy.deepcopy(shf))
...
#        libc.gen_entire(byref(rot_array),byref(xyzcent_t_array),byref(x1),byref(y1),byref(z1),byref(x2),byref(y2),byref(z2),c_int(n),c_int(i))
        ...
        shift  = shfs[i]
        for k in s2:
            x2[k] = matrix[0][0]*x1[k]+matrix[0][1]*y1[k]+matrix[0][2]*z1[k]+shift[0]
            y2[k] = matrix[1][0]*x1[k]+matrix[1][1]*y1[k]+matrix[1][2]*z1[k]+shift[1]
            z2[k] = matrix[2][0]*x1[k]+matrix[2][1]*y1[k]+matrix[2][2]*z1[k]+shift[2]
